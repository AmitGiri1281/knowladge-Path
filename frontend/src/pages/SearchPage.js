    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <form onSubmit={handleSearch} className="max-w-2xl mx-auto mb-8">
        ...
        <div className="flex shadow-lg rounded-full overflow-hidden border-2 border-blue-100 focus-within:border-blue-500 transition">
          ...
          />
          {searchInput && (
            <button type="button" onClick={clearSearch} className="px-3 text-gray-400 hover:text-gray-600">
              <FaTimes />
            </button>
          )}
          <button
      ...
      <div className="max-w-5xl mx-auto">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {results.map((result) => (
              <Link key={result._id} to={`/content/${result._id}`} aria-label={`View ${result.title}`}>
